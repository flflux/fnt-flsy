...
export function EditVehicleCard({ open, onClose, onUpdate, initialData }: EditVehicleCardProps) {
  ...
  const initialCardNum = initialData?.number ?? null;
  const initialCardType = initialData?.type ?? null;

  const handleUpdate = (data: AddCard) => {
    onUpdate(data);
    setValue('number', initialCardNum ?? null as unknown as string );
    setValue('type', initialCardType ?? null as unknown as string);
    onClose();
  };
