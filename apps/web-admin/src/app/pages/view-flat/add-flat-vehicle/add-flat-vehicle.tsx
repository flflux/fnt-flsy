...
const AddFlatVehicleComponent: React.FC<AddFlatVehicleProps> = ({ open, onClose, onSubmit, initialData }) => {
  ...
  const validationSchema = yup.object().shape({
    ...
    number: yup.string().matches(/^[A-Z]{2}[0-9]{2}[A-HJ-NP-Z]{1,2}[0-9]{4}$|^[0-9]{2}BH[0-9]{4}[A-HJ-NP-Z]{1,2}$/, 'Invalid vehicle number').required('Vehicle Number is required'),
  ...
  const { handleSubmit, control, reset, formState: { errors }, setValue, watch } = useForm<Form>({
  ...
  useEffect(() => {
    if (open) {
      reset();
      setValue('buildingId', initialData?.floor.building.id || 0);
      setValue('floorId', initialData?.floor.id || 0);
      setValue('flatId', initialData?.id || 0);
    }
  }, [open, reset]);
  ...
  useEffect(() => {
    ...
    getAllFloors();
    getAllFlats();
  }, [page, rowsPerPage, totalbuildingValue, totalFlatValue, totalValue, selectedBuildingId, selectedFloorId]);
