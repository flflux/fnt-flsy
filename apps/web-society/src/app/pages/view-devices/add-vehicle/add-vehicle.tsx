...
interface AddVehicleProps {
  ...
  newAddVehicle: (lastNewVehicleId: NewVehicleId) => void;
}
...
const AddVehicleComponent: React.FC<AddVehicleProps> = ({ open, onClose, onSubmit, onExistSubmit, newAddVehicle }) => {
  ...
  useEffect(() => {
    getAllVehicles();
  }, [user, totalVehicleValue, societycontext]);
      ...
      if (data) {
        ...
        try {
          ...
          if (response.data.content.length > 0) {
            const lastNewVehicleId = response.data.content[response.data.content.length - 1].id;
            console.log("Last New Vehicle ID:", lastNewVehicleId);
            await newAddVehicle({ vehicleId: lastNewVehicleId as number });
          }
