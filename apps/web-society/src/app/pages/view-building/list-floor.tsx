...
export function ListFloor(props: ListFloorProps) {
  ...
  const getFloors = async () => {
    try {
      setLoading(true);
      const response = await axios.get(`${apiUrl}/societies/${societycontext?.id}/buildings/${params.id}/floors`, {
  ...
  const handleAddFloor = async (formData: Form) => {
    try {
      const { data } = await axios.post(`${apiUrl}/societies/${societycontext?.id}/buildings/${formData.buildingId}/floors`, { number: formData.number },
        {
          withCredentials: true,
        },)
      if (data) {
  ...
  const handleUpdate = async (data: Form) => {
    try {
      const response = await axios.put(
        `${apiUrl}/societies/${societycontext?.id}/buildings/${data.buildingId}/floors/${selectedFloorId}`,
        { number: data.number },
        {
          withCredentials: true,
          headers: {
  ...
  const handleDelete = async (floor: { id: number, buildingId: number } | null) => {
    try {
      const { data } = await axios.delete(`${apiUrl}/societies/${societycontext?.id}/buildings/${floor?.buildingId}/floors/${floor?.id}`, {
        withCredentials: true,
      });
      console.log(data);
  ...
  useEffect(() => {
    const getBuildingDetails = async () => {
      try {
        const response = await axios.get(`${apiUrl}/societies/${societycontext?.id}/buildings/${params.id}`, {
          withCredentials: true,
        });
  ...
  const breadcrumbs = [
    ...
    {
      to: `/society/${societycontext?.id}/buildings`,
      label: "Buildings",
    },
    {
      label: `${buildingName}`,
                ...
                {loading ? (
                  ...
                  activeFloors.map((floor: ViewFloor) => (
                    <TableRow >
                      ...
                      <TableCell>
                        <IconButton  sx={{color:'black'}} className={styles['row-action-button']} onClick={() => handleEditClick(floor.id)}>
                        <EditIcon>
                          Edit
                        </EditIcon>
                        </IconButton>
